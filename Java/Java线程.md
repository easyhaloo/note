...
![1562685916139](assets/1562685916139.png)
...
4.  **Waiting和Blocked的区别**

    **Blocked**的状态是指，当一个线程请求某个资源（Lock,IO,Socket等）失败时，会进入阻塞态，在Java中，阻塞态专指请求锁失败时进入的状态，Java内部存在一个阻塞队列专门用来存放阻塞的线程。处于阻塞态的线程会不断请求资源，一旦请求资源成功，则会进入就绪队列（Ready Queue）等待执行。

    **Waiting**状态是指，线程调用`wait、join、park`函数时，线程进入等待态，处于等待态的线程会被存放到等待队列中，同时会释放CPU执行权，并释放资源(Lock)。等待队列中的线程需要等待其他线程释放信号（notify、notifyAll）才可以继续运行。

    **Timed_Waiting** 和**Waiting**有点类似，它是主动进入等待，需要等待其他线程唤醒，但是与**Waiting**不一样的是，等到到达超时时间后，会自动进入阻塞队列，开始锁竞争。
...
5.  **后台线程**
    ...
    Java中的线程分为两类，一类是普通线程即使用`new`创建出来的线程，还有一类线程是通过调用`Thread.setDaemon(true)`表示的守护线程，也称作为后台线程。守护线程的特点是，当JVM观察到只有守护线程运行时，JVM将自动关闭。也就是守护线程以来普通线程而存活，不能独立存在。还有一点值得注意的是，JVM中如果`Main`线程执行完毕后，新开启的普通线程并不会立即中断或是退出，而是等到线程执行完毕才退出。

6.  **ThreadLocal**

     Java线程中，每个Thread都有一个ThreadLocal的属性，能够保证在整个线程的周期中，ThreadLocal与正在运行的线程绑定。我们一般想要保证线程安全，那么我们可以借助于ThreadLocal。`ThreadLocal`的内部机制你可以看作是一个`HashMap`,其`key`为当前线程，`value`需要存入的值。

    **Map<Thread,Object>**。

    `ThreadLocal`经常用来保存一些`事物ID`、`Cookie`、`Request请求信息`等上下文相关信息。

    使用`ThreadLocal`需要注意的事项：当不用`ThreadLocal`的变量时，需要及时的清空。`ThreadLocal`中保存的对象，如果不显示清空，那么将会等到线程执行结束，触发GC操作才能正式回收内存。很多时候我们采用的线程池来管理线程，通过线程池来管理的线程，基本上永远都不会结束。这意味着，你使用了`ThreadLocaL`来保存的对象不会显示的进行垃圾回收。那么这将造成`OOM`异常。

7.   **创建线程的几种方式？**

     对于网上所阐述的三种，乃至四种方式一直不是很懂。很多人都认为实现`Runable，Callable`后也是一种创建线程方式，一直不是很理解。在我的认知里面，`Runable，Callable`这两者只能算是线程中抽象出来的任务接口。主要用来实现业务分离。实际上还是需要手动创建线程。还有两种实现方式：
...
[Java线程状态和生命周期](https://www.uml-diagrams.org/java-thread-uml-state-machine-diagram-example.html)
